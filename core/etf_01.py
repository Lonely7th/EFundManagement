...
class EmaManager:
    ...
    def fun_01(self):
        ...
        for code_item in code_list:
            ...
            # 获取数据
            tk_details = self.db_manager_tk.find_one_by_key({"ticker": ticker})
            close_list = [float(x["close"]) for x in tk_details["price_list"] if x["close"] != ""]
            open_list = [float(x["open"]) for x in tk_details["price_list"] if x["close"] != ""]
            volume_list = [float(x["volume"]) for x in tk_details["price_list"] if x["close"] != ""]
            ema_list = list()
